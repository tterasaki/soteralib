...
def main_Run11(obs_id):
    ...
    # get SNR_man
    SNR_man = core.AxisManager.load(f'/so/home/tterasaki/summary/2402_abscal/MF2_Run11/beaminfo_Run11_wmapSNR/{obs_id}.hdf')
    aman.restrict('dets', SNR_man.dets.vals)
    aman.wrap('SNR', SNR_man.SNR, [(0, 'dets')])
    aman.restrict('dets', aman.dets.vals[aman.SNR > 3.], [(0, 'dets')])
    aman.restrict('dets', aman.dets.vals[~np.isnan(aman.biasstep.si)])
    ...
    # TOD processing
    tod_process_v1(aman, apply_fourier_filt=True, remove_glitchy=False) #, glitchy_th=0.01)
    
    # map making
    res_deg = 0.1
    save_dir = '/so/home/tterasaki/summary/2402_abscal/MF2_Run11/coadded_map_Run11/'
